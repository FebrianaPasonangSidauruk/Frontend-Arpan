...
function TesProject() {
  ...
  const [val, setVal] = useState();
    ...
    useEffect(() => {
      ...
      const getProjects = async () => {
        const response = await axios.get(
          `http://localhost:5010/projects?search_query=${keyword}&page=${page}&limit=${limit}`
        );
    ...
    const nih =async(projid) => {
        ...
        console.log(id_project)
        const response = await axios.get(`http://localhost:5010/datas/${projid}`);
